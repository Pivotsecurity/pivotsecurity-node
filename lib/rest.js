...
function Request(uri, options) {
  ...
  this.headers = {
    'Accept': '*/*',
    'User-Agent': 'Pivot Security API Node.js/1.0.0',
    'Host': this.url.host
  };
  ...
  if (this.options.multipart) {
    ...
    if (typeof this.options.data == 'string') {
      var buffer = Buffer.from(this.options.data, this.options.encoding || 'utf8');
...
mixin(Request.prototype, {
  ...
  _applyAuth: function() {
    ...
    if (this.options.username && this.options.password !== undefined) {
      var b = Buffer.from([this.options.username, this.options.password].join(':'));
  ...
  _responseHandler: function(response) {
    ...
    if (self._isRedirect(response) && self.options.followRedirects) {
      ...
      response.on('end', function() {
        ...
        self._decode(Buffer.from(body, 'binary'), response, function(err, body) {
          if (err) {
            self._fireError(err, response);
            return;
          }
...
function postJson(url, data, options, key) {
  return json(url, data, options, 'POST', key);
}
